...
/* Visual highlighting for benchmark columns */
.spectral-table-html thead th.benchmark-column {
...
/* Hide theta-hat, uniform CI, and average score rank columns when details are shown */
.spectral-table-html table.show-details .theta-hat-col,
...
def _load_arena_benchmark_matrix():
    ...
    benchmark_file = os.path.join(PROJECT_ROOT, 'data_llm', 'data_arena', 'data_processing', 'arena_elo_full.csv')
...
def load_arena_combination_spectral_results(selected_virtual_keys: list, base_spectral_results: dict = None) -> dict:
    """Load spectral results for a selected Arena combination (1-6). If 7, return full results.
    ...
    # 1-6 selection: load from all_combinations
    benchmarks_virtual, model_names, scores_matrix = _load_arena_benchmark_matrix()
    # Validate that all selected keys exist in the benchmark matrix
    invalid_keys = [key for key in selected_virtual_keys if key not in benchmarks_virtual]
    if invalid_keys:
        raise FileNotFoundError(f'Unrecognized benchmark keys: {invalid_keys}')
    # Convert virtual keys to field names and sort to match directory naming convention
    field_names = [ARENA_VIRTUAL_TO_FIELD[key] for key in selected_virtual_keys]
    field_names_sorted = sorted(field_names)
    combination_name = "_".join(field_names_sorted)
    ...
    with open(result_path, 'r') as f:
        raw_spectral_results = json.load(f)

    # Convert array format to expected object format with methods (if needed)
    if isinstance(raw_spectral_results, list):
        spectral_results = {'methods': []}
        for item in raw_spectral_results:
            method = {
                'name': item['model'],
                'theta_hat': item['theta_hat'],
                'rank': item['rank'],
                'ci_two_sided': [item['ci_two_left'], item['ci_two_right']],
                'ci_left': item['ci_left'],
                'ci_uniform_left': item['ci_uniform_left']
            }
            spectral_results['methods'].append(method)
    else:
        spectral_results = raw_spectral_results

    # Attach benchmark_scores (all 7 fields) and average for selected set
    vkey_to_idx = {v: i for i, v in enumerate(benchmarks_virtual)}
    selected_indices = [vkey_to_idx[v] for v in selected_virtual_keys]
    updated_methods = []
...
def create_ranking_table(data, highlight_model: str = None, is_arena_mode: bool = False):
            ...
            with ui.element('div').classes('benchmark-selection-card'):
                with ui.element('div').classes('benchmark-selection-header'):
                    ...
                    with ui.element('div'):
                        ...
                        ui.html('<div class="benchmark-selection-subtitle">Choose which virtual benchmarks to include in your spectral ranking analysis. You can select 1 to 7 benchmarks.</div>')
                ...
                def on_confirm_click():
                    try:
                        ...
                        with ranking_content_container:
                            if len(selected_virtual) < 1:
                                ui.html('<div style="padding:1rem; border:1px dashed #e2e8f0; border-radius:8px; color:#b91c1c; background:#fef2f2;">Please select at least one benchmark.</div>')
                                return
...
def create_spectral_ranking_table(data, spectral_results, highlight_model: str = None, selected_benchmarks: list = None):
    ...
    if is_arena:
        ...
        all_benchmark_columns = [
            {'name': 'creative_writing', 'label': 'Creative Writing', 'field': 'creative_writing', 'align': 'left', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'The model\'s rank in the "Creative Writing" category. This category evaluates the ability to generate original, imaginative, and emotionally resonant content based on human preference votes.'},
            {'name': 'math', 'label': 'Math', 'field': 'math', 'align': 'left', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'The model\'s rank in the "Math" category. This category evaluates the ability to apply mathematical reasoning and problem-solving skills based on human preference votes.'},
            {'name': 'instruction_following', 'label': 'Instruction Following', 'field': 'instruction_following', 'align': 'left', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'The model\'s rank in the "Instruction Following" category. This category evaluates the ability to accurately follow specific and detailed user instructions based on human preference votes.'},
            {'name': 'coding', 'label': 'Coding', 'field': 'coding', 'align': 'left', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'The model\'s rank in the "Coding" category. This category evaluates the ability to understand, generate, and debug code based on human preference votes.'},
            {'name': 'hard_prompt', 'label': 'Hard Prompt', 'field': 'hard_prompt', 'align': 'left', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'The model\'s rank in the "Hard Prompt" category. This category evaluates the ability to handle complex, rigorously designed prompts that require multiple skills, based on human preference votes.'},
            {'name': 'longer_query', 'label': 'Longer Query', 'field': 'longer_query', 'align': 'left', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'The model\'s rank in the "Longer Query" category. This category includes user prompts that are longer than 500 tokens, testing long-context understanding.'},
            {'name': 'multi_turn', 'label': 'Multi-turn', 'field': 'multi_turn', 'align': 'left', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'The model\'s rank in the "Multi-Turn" category. This category evaluates performance in conversational interactions that involve more than one turn.'},
        ]
        ...
        all_hf_benchmark_columns = [
            {'name': 'ifeval', 'label': 'IFEval', 'field': 'ifeval', 'align': 'left', 'style': 'width: 70px;', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'Instruction Following Evaluation (IFEval): Assesses the model\'s ability to follow complex and detailed instructions, focusing on precision and adherence to constraints, not creativity.'},
            {'name': 'bbh', 'label': 'BBH', 'field': 'bbh', 'align': 'left', 'style': 'width: 70px;', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'Big-Bench Hard (BBH): A challenging subset of the Big-Bench benchmark, featuring 23 tasks that require significant multi-step reasoning abilities from the language models.'},
            {'name': 'math', 'label': 'MATH', 'field': 'math', 'align': 'left', 'style': 'width: 70px;', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'A benchmark consisting of 12,500 challenging competition mathematics problems from high school level contests, designed to test mathematical problem-solving and reasoning.'},
            {'name': 'gpqa', 'label': 'GPQA', 'field': 'gpqa', 'align': 'left', 'style': 'width: 70px;', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'Graduate-Level Google-Proof Q&A (GPQA): A difficult dataset of questions written by domain experts that are hard to find answers for using search engines, testing deep domain knowledge.'},
            {'name': 'musr', 'label': 'MUSR', 'field': 'musr', 'align': 'left', 'style': 'width: 70px;', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'Multi-Step Reasoning (MuSR): Evaluates the model\'s ability to perform complex, multi-step reasoning by solving problems that require chaining together facts and inferences.'},
            {'name': 'mmlu_pro', 'label': 'MMLU-Pro', 'field': 'mmlu_pro', 'align': 'left', 'style': 'width: 70px;', 'sortable': True, 'class': 'benchmark-column', 'tooltip': 'An advanced version of the MMLU benchmark that features more challenging questions requiring deeper knowledge and reasoning, curated by subject matter experts.'},
        ]
    ...
    with ui.element('div').style('max-height: 600px; overflow: auto; border: 1px solid #e0e0e0; border-radius: 8px; position: relative;').classes('spectral-table-container'):
...
def load_spectral_results(arena=False):
    ...
    try:
        if arena:
            # Load Arena spectral results from ranking_results.json
            ranking_file = os.path.join(PROJECT_ROOT, 'data_llm', 'data_arena', 'data_ranking', 'current', 'ranking_results.json')
            if os.path.exists(ranking_file):
                with open(ranking_file, 'r') as f:
                    raw_data = json.load(f)

                # Convert array format to expected object format with methods
                converted_data = {
                    'methods': []
                }
                ...
                for item in raw_data:
                    ...
                    converted_data['methods'].append(method)

                logger.info("Loaded and converted Arena spectral ranking results")
    ...
    with ui.element('section').classes('hero-section'):
        with ui.element('div').classes('hero-content'):
            ui.html('''
                ...
                <h1 class="hero-title">SpectralRank LLM Leaderboard</h1>
            ...
            with ui.element('div').classes('hero-features').style('display: grid; grid-template-columns: repeat(4, 1fr); gap: 1.5rem;'):
                ...
                ''')):
                    ...
                    ui.html('<p class="hero-feature-description">Delivers statistically robust LLM rankings by analyzing head-to-head comparison data, providing reliable results that reflect actual model performance nuances.</p>')
                ...
                ''')):
                    ...
                    ui.html('<h3 class="hero-feature-title">Multi Data Sources</h3>')
                    ui.html('<p class="hero-feature-description">Access rankings from LMSYS Arena (crowdsourced preferences) and Hugging Face (objective benchmarks) for comprehensive LLM evaluation.</p>')

                with ui.element('div').classes('hero-feature').style('cursor: pointer;').on('click', lambda: ui.run_javascript('''
                    // First switch to Arena mode if not already active
                    ...
                    if (arenaContent && arenaContent.style.display === 'none') {
                        ...
                        if (huggingfaceContent) {
                            huggingfaceContent.style.display = 'none';
                        }
                    ...
                    // Then scroll to the Arena rankings section
                    setTimeout(function() {
                        const target = document.getElementById('arena-rankings');
                        if (target) {
                            const navbar = document.querySelector('.top-navbar');
                            const navbarHeight = navbar ? navbar.offsetHeight : 60;
                            const targetRect = target.getBoundingClientRect();
                ...
                ''')):
                    ...
                    ui.html('<h3 class="hero-feature-title">Custom Rankings</h3>')
                    ui.html('<p class="hero-feature-description">Select specific benchmarks to create personalized rankings that match your use cases.</p>')

                with ui.element('div').classes('hero-feature').style('cursor: pointer;').on('click', lambda: ui.run_javascript('''
                    // First switch to HuggingFace mode if not already active
                    const huggingfaceContent = document.getElementById('huggingface-content');
                    const arenaContent = document.getElementById('arena-content');
                ...
                ''')):
                    ...
                    ui.html('<h3 class="hero-feature-title">Rank Your Model</h3>')
                    ui.html('<p class="hero-feature-description">Input your model\'s benchmark scores to get comprehensive statistical ranking analysis against top LLMs.</p>')
        ...
        with ui.element('div').classes('grid-container').classes('intro-grid-three-columns').style('grid-template-columns: repeat(3, 1fr);'):
                ...
                ui.html('''
                    <div class="card-description">
                        ...
                        <ul>
                            ...
                            <li><span class="material-symbols-outlined">tune</span><div class="benchmark-item"><strong>Customizable Analysis:</strong> Select 1-7 benchmarks to create your own focused ranking tailored to specific performance areas.</div></li>
            ...
            with ui.element('div').classes('step-card compact-card unified-header intro-card'):
                ...
                ui.html('''
                    <div class="card-description">
                        <ul>
                            <li><span class="material-symbols-outlined">dataset</span><div class="benchmark-item"><strong>Multi Data Sources:</strong> Access rankings based on both human preference data (LMSYS Arena) and standardized benchmarks (Hugging Face), giving you multiple perspectives on model performance.</div></li>
                            <li><span class="material-symbols-outlined">tune</span><div class="benchmark-item"><strong>Custom Rankings:</strong> Choose which benchmarks matter most to you. Select 1-7 benchmarks to create a customized ranking focused on your specific use case or evaluation criteria.</div></li>
...
def create_huggingface_content(data):
    ...
    with ui.element('section').style('width: 100%; max-width: 1400px; margin: 0 auto; padding: 0 2rem;'):
        ...
        ui.html('<h2 class="section-title">Hugging Face LLM Rankings</h2>')
    ...
    with ui.element('section').style('width: 100%; max-width: 1400px; margin: 2rem auto; padding: 0 2rem;'):
        ...
        with ui.element('div').classes('grid-container'):
            ...
            with ui.element('div').classes('step-card'):
                ...
                ui.html(r'''
                    <div class="card-description">
                        <p>This is the final step that takes all the scores from the benchmarks you selected (you can choose anywhere from 1 to 6) and combines them into one final ranking using advanced math called the <strong>Vanilla Spectral Method</strong>. This creates a more reliable and fair overall ranking than just averaging the scores.</p>
                        <ul>
...
def create_arena_content(data):
    ...
    # Rankings Table
    with ui.element('section').style('width: 100%; max-width: 1400px; margin: 0 auto; padding: 0 2rem;').props('id="arena-rankings"'):
        # Determine the number of models being displayed
        spectral_results = data.get('spectral_results')
        if spectral_results and 'methods' in spectral_results:
            display_count = len(spectral_results['methods'])
    ...
    with ui.element('section').style('width: 100%; max-width: 1400px; margin: 2rem auto; padding: 0 2rem;'):
        ...
        with ui.element('div').classes('grid-container'):
            ...
            with ui.element('div').classes('step-card'):
                ...
                ui.html(r'''
                    <div class="card-description">
                        <p>This is the final step that takes all the scores from the virtual benchmarks you selected (you can choose anywhere from 1 to 7) and combines them into one final ranking using advanced math called the <strong>Vanilla Spectral Method</strong>. This creates a more reliable and fair overall ranking than just averaging the scores.</p>
                        <ul>
                            <li><span class="material-symbols-outlined">hub</span><div class="benchmark-item"><strong>Core Idea: A "Tournament Network"</strong>
    ...
    with ui.element('div').classes('dashboard-container').style('min-height: 100vh; width: 100vw; display: flex; flex-direction: column; align-items: center; padding: 0; margin: 0;'):
        ...
        with ui.element('nav').classes('top-navbar'):
            ...
            with ui.element('div').classes('navbar-brand'):
                ...
                ui.html('<a href="#" onclick="window.location.href=\'/\'" class="navbar-brand-link">SpectralRank</a>')
