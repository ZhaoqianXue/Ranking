...
class ArenaRankingDataPreparer:
    ...
    def save_ranking_data(self, df, filename=None):
        ...
        if filename is None:
            ...
            filename = f'arena_elo_{timestamp}.csv'
...
def main():
    ...
    if LIMIT_MODEL_COUNT:
        ...
        for top_n in TOP_DATASETS_TO_GENERATE:
            if len(model_metrics) >= top_n:
                ...
                ranking_df_top, top_models = preparer.prepare_ranking_data(model_metrics,
                                                                         ...
                                                                         top_n=top_n)
                top_filepath = preparer.save_ranking_data(ranking_df_top, f'arena_elo_top{top_n}.csv')
        ...
        ranking_df_full, all_models_sorted = preparer.prepare_ranking_data(model_metrics,
                                                                          list(model_metrics.keys()),
                                                                          top_n=None)
        full_filepath = preparer.save_ranking_data(ranking_df_full, 'arena_elo_full.csv')
    ...
    for dataset in generated_datasets:
        if 'type' in dataset and dataset['type'] == 'full':
            ...
            print("  --csv data_llm/data_arena/data_processing/arena_elo_full.csv \\")
            print("  --bigbetter 1 --B 2000 --seed 42 \\")
            print("  --out data_llm/data_arena/data_ranking/arena_elo_full")
        else:
            print(f"\n# Top {dataset['top_n']} models:")
            print("Rscript demo_r/ranking_cli.R \\")
            print(f"  --csv data_llm/data_arena/data_processing/arena_elo_top{dataset['top_n']}.csv \\")
            print("  --bigbetter 1 --B 2000 --seed 42 \\")
            print(f"  --out data_llm/data_arena/data_ranking/arena_elo_top{dataset['top_n']}")
