#!/usr/bin/env python3
"""
Update Dashboard Ranking Data for HuggingFace Human Preferences - All Combinations Version

This script runs SPECTRAL RANKING on ALL POSSIBLE COMBINATIONS of HuggingFace benchmarks,
producing spectral rankings for each combination of 2 to 5 benchmarks (excluding the full 6-benchmark combination).

Usage:
    python huggingface_ranking_single.py [--bigbetter 1] [--B 2000] [--seed 42]

The script will:
1. Load the full HuggingFace ranking data (6 benchmarks)
2. Generate all combinations of 2-5 benchmarks (56 combinations total, excluding full 6-benchmark combo)
...
class HuggingFaceAllCombinationsRankingUpdater:
    """Updates ranking data for all possible combinations of HuggingFace benchmarks"""
    ...
    def __init__(self):
        ...
        self.input_file = os.path.join(self.data_llm_dir, 'huggingface_processed_top100.csv')

        # Ensure ranking directory exists
        os.makedirs(self.data_ranking_dir, exist_ok=True)

    def load_full_data(self) -> pd.DataFrame:
        """Load the full HuggingFace ranking data"""
        if not os.path.exists(self.input_file):
            raise FileNotFoundError(f"Input file not found: {self.input_file}")

        logger.info(f"Loading full HuggingFace data: {self.input_file}")
    ...
    def create_combination_data(self, df: pd.DataFrame, benchmark_combination: List[str]) -> str:
        ...
        temp_fd, temp_path = tempfile.mkstemp(suffix='.csv', prefix=f'huggingface_{combination_name}_')
        os.close(temp_fd)
    ...
    def run_all_combinations(self, bigbetter: int = 1, B: int = 2000, seed: int = 42, max_combinations: int = None) -> Dict[str, Dict]:
        ...
        logger.info("Starting all-combinations ranking for HuggingFace benchmarks...")
        ...
        print(f"\n🚀 STARTING HUGGINGFACE ALL-COMBINATIONS SPECTRAL RANKING")
        ...
        print(f"⏱️  Estimated total runtime: ~{total_combinations * 8:.0f}-{total_combinations * 15:.0f} seconds (8-15s per combination)")
        print("=" * 80)
        ...
        try:
            ...
            for i, combination in enumerate(all_combinations, 1):
                ...
                progress_percent = (i - 1) / len(all_combinations) * 100
                elapsed_time = time.time() - start_time
                avg_time_per_combination = elapsed_time / (i - 1) if i > 1 else 0
                estimated_remaining = avg_time_per_combination * (len(all_combinations) - i + 1)

                print(f"\n🔄 Progress: {i}/{len(all_combinations)} combinations ({progress_percent:.1f}%)")
                ...
                progress_percent = completed_combinations / len(all_combinations) * 100

                print(f"✅ Completed: {combination_name} ({completed_combinations}/{len(all_combinations)} - {progress_percent:.1f}%)")
    ...
    def save_combined_results(self, all_results: Dict[str, Dict]):
        ...
        # Save combined results for all combinations
        combined_file = os.path.join(output_dir, 'huggingface_all_combinations_rankings.json')
        ...
        summary_file = os.path.join(output_dir, 'huggingface_combinations_summary.csv')
    ...
    def create_summary_table(self, all_results: Dict[str, Dict]) -> pd.DataFrame:
        ...
        summary_file = os.path.join(output_dir, 'huggingface_single_benchmark_summary.csv')
        summary_df.to_csv(summary_file, index=False)

        logger.info(f"Saved summary table: {summary_file}")
        return summary_df

    def print_combinations_summary(self, all_results: Dict[str, Dict]):
        """Print a summary of all combinations results"""
        print("\n" + "="*80)
        print("HUGGINGFACE ALL-COMBINATIONS SPECTRAL RANKING SUMMARY")
        print("="*80)

        print(f"\nProcessed {len(all_results)} combinations:")

        # Group by combination size
        by_size = {}
        for combination_name, results in all_results.items():
            n_benchmarks = results['n_benchmarks']
            if n_benchmarks not in by_size:
                by_size[n_benchmarks] = []
            by_size[n_benchmarks].append((combination_name, results))

        for size in sorted(by_size.keys()):
            combinations = by_size[size]
            print(f"\n{size} benchmarks ({len(combinations)} combinations):")
            for combination_name, results in combinations[:3]:  # Show first 3
                top_model = results['methods'][0]['name'] if results['methods'] else "N/A"
                print(f"  - {combination_name}: Top model = {top_model}")
            if len(combinations) > 3:
                print(f"  ... and {len(combinations) - 3} more")

        # Show overall statistics
        total_runtime = sum(results['metadata']['runtime_sec'] for results in all_results.values())
        avg_runtime = total_runtime / len(all_results) if all_results else 0

        print("\nOverall Statistics:")
        print(f"  - Total combinations: {len(all_results)}")
        print(f"  - Total runtime: {total_runtime:.1f} seconds")
        print(f"  - Average runtime per combination: {avg_runtime:.1f} seconds")
        print(f"  - Bootstrap iterations: {list(all_results.values())[0]['params']['B'] if all_results else 'N/A'}")

        print("\nTop 3 combinations by runtime:")
        sorted_by_runtime = sorted(all_results.items(), key=lambda x: x[1]['metadata']['runtime_sec'], reverse=True)
        for i, (combination_name, results) in enumerate(sorted_by_runtime[:3], 1):
            runtime = results['metadata']['runtime_sec']
            n_benchmarks = results['n_benchmarks']
            print(f"{i}. {combination_name} ({n_benchmarks} benchmarks): {runtime:.1f}s")
    ...
    def update_ranking(self, bigbetter: int = 1, B: int = 2000, seed: int = 42, max_combinations: int = None):
        ...
        logger.info("="*80)
        logger.info("STARTING HUGGINGFACE ALL-COMBINATIONS SPECTRAL RANKING")
        logger.info("="*80)
        ...
        try:
            ...
            self.print_combinations_summary(all_results)

            logger.info("="*80)
            logger.info("HUGGINGFACE ALL-COMBINATIONS SPECTRAL RANKING COMPLETED SUCCESSFULLY")
            logger.info("="*80)

            return combined_file

        except Exception as e:
            logger.error(f"HuggingFace all-combinations ranking update failed: {e}")
            raise


def main():
    parser = argparse.ArgumentParser(description="Update HuggingFace ranking data with all-combinations spectral ranking")
    ...
    # Run update
    updater = HuggingFaceAllCombinationsRankingUpdater()
