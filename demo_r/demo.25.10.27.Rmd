---
title: "Demo for method ranking project"
...
n <- 5000   # there are n samples
k <- 5     # there are k methods
na_frac <- 0.05  # there are 5% of NA`s 
rankings <- 1:5  # default ranking
variances <- rep(0.5, k) # each method have some variance
...
print(rr)
write.csv(as.data.frame(rr), file.path(getwd(), "ranking_output.csv"), row.names = FALSE)
```
...
```{r}
### test on real arena data ###
data <- read_csv(file.path(getwd(), "arena_test_data.csv"), show_col_types = FALSE)
