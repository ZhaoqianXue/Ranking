...
vanilla_spectrum_method <- function(AA2, WW2, Idx, B = 2000) {
  ...
  timing$matrix_construction_start <- Sys.time()
  ...
  timing$bootstrap_start <- Sys.time()
  ...
  timing$bootstrap_end <- Sys.time()

  # Uniform left-sided CI computation
  timing$uniform_ci_start <- Sys.time()
  ...
  timing$uniform_ci_end <- Sys.time()
  ...
  timing$matrix_construction_time <- as.numeric(difftime(timing$matrix_construction_end, timing$matrix_construction_start, units = "secs"))
  timing$svd_time <- as.numeric(difftime(timing$svd_end, timing$svd_start, units = "secs"))
  timing$bootstrap_time <- as.numeric(difftime(timing$bootstrap_end, timing$bootstrap_start, units = "secs"))
  timing$uniform_ci_time <- as.numeric(difftime(timing$uniform_ci_end, timing$uniform_ci_start, units = "secs"))
  timing$total_spectral_time <- timing$matrix_construction_time + timing$svd_time + timing$bootstrap_time + timing$uniform_ci_time

  list(RR2 = RR2, timing = timing)
}
...
main <- function() {
  ...
  io_time <- as.numeric(difftime(Sys.time(), io_start, units = "secs"))
  ...
  spectral_start <- Sys.time()
  result <- vanilla_spectrum_method(pdata$aa, pdata$ww, pdata$idx, B = B)
  RR2 <- result$RR2
  spectral_timing <- result$timing
  spectral_time <- as.numeric(difftime(Sys.time(), spectral_start, units = "secs"))
  ...
  payload <- list(
    ...
    metadata = list(
      ...
      detailed_timing = list(
        io_time = io_time,
        preprocess_time = preprocess_time,
        spectral_time = spectral_time,
        matrix_construction_time = spectral_timing$matrix_construction_time,
        svd_time = spectral_timing$svd_time,
        bootstrap_time = spectral_timing$bootstrap_time,
        uniform_ci_time = spectral_timing$uniform_ci_time,
        total_spectral_time = spectral_timing$total_spectral_time
      )
    )
  )
